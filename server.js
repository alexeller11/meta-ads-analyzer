...
// ROTA DE SALDO (BALANCE) - ADICIONADA
...
// ROTA DE CRIATIVOS (CREATIVES) - ADICIONADA
  ...
  try {
    ...
    const enriched = campaigns.map(c => {
      ...
      if (m.sp > 0) {
          ...
          else if (campRoas > 1.2) diagIA = "✅ ROI Positivo. Mantenha a campanha.";
          else if (parseFloat(m.ctr) < 0.8) diagIA = "🪝 CTR Baixo. Troque o criativo.";
      }
    ...
    const aiAnalysis = runAnalysisEngine(accountData, enriched, metrics);
    res.json({ success: true, analysis: aiAnalysis, metrics });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

function runAnalysisEngine(accountData, campaigns, metrics) {
  const { avgCtr, totalSpend, connectRate, roas } = metrics;
  let score = totalSpend > 0 ? 100 : 0;
  const otimizacoes = [];
  if (avgCtr < 1.0) { score -= 20; otimizacoes.push({ prioridade: 1, titulo: 'Fadiga Criativa', categoria: 'Criativo', descricao: 'Seu CTR está abaixo de 1%.', acao: 'Troque os criativos.' }); }
  if (connectRate < 60 && totalSpend > 50) { score -= 15; otimizacoes.push({ prioridade: 2, titulo: 'Lentidão no Funil', categoria: 'Site', descricao: 'Connect Rate baixo.', acao: 'Otimize a velocidade do site.' }); }
  return { resumo_geral: { score_saude: Math.max(0, score), nivel_saude: score > 80 ? 'Excelente' : 'Atenção' }, campanhas_analise: campaigns, otimizacoes_prioritarias: otimizacoes };
}
