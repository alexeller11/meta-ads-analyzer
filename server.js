...
app.get('/api/adaccounts/:id/insights', auth, async (req, res) => {
  try {
    ...
    const params = { fields: 'campaign_id,campaign_name,impressions,clicks,spend,cpc,cpm,ctr,actions,action_values,frequency', level: 'campaign', access_token: req.session.accessToken, limit: 200 };
...
app.post('/api/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    let tSpend = 0, tImpr = 0, tClicks = 0, tSess = 0, tRev = 0, tPur = 0, tLds = 0, tMsg = 0;
    const byId = {};
    ...
    const enriched = campaigns.map(c => {
      ...
      return { ...c, spend: m.sp, ctr: parseFloat(m.ctr || 0), cpc: parseFloat(m.cpc || 0), impressions: m.impr, clicks: m.cl, purchases: m.pur, messages: m.msg, revenue: m.rev, roas: m.sp > 0 ? m.rev / m.sp : 0, connectRate: m.cl > 0 ? (m.sess / m.cl) * 100 : 0 };
    });
...
function runAnalysisEngine(accountData, campaigns, metrics, previousRun) {
  ...
  if (avgCtr < 1.0) {
      score -= 20;
      otimizacoes.push({ prioridade: 1, titulo: 'Atenção Criativa Baixa', categoria: 'Criativo', impacto_esperado: 'Alto', descricao: 'O público não está a clicar.', acao: 'Ação: Troque os ganchos dos seus anúncios mais gastadores.' });
  }
  if (connectRate < 50 && totalSpend > 50) {
      score -= 15;
      otimizacoes.push({ prioridade: 2, titulo: 'Vazamento no Site', categoria: 'Funil', impacto_esperado: 'Crítico', descricao: 'Muitos cliques não carregam a página.', acao: 'Ação: Melhore a velocidade do site mobile.' });
  }

  const campanhas_analise = campaigns.map(c => ({
    nome: c.name, gasto: `${S} ${c.spend.toFixed(2)}`, ctr: `${c.ctr.toFixed(2)}%`, cpc: `${S} ${c.cpc.toFixed(2)}`,
    roas: c.roas > 0 ? `${c.roas.toFixed(2)}x` : '-', mensagens: c.messages, connectRate: `${c.connectRate.toFixed(1)}%`,
    diagnostico: c.roas > 2.5 ? 'Escalar' : c.ctr < 1 ? 'Trocar Criativo' : 'Estável', spendRaw: c.spend, campStatus: c.status
  }));

  return { resumo_geral: { score_saude: Math.max(0, score), nivel_saude: score > 80 ? 'Excelente' : 'Atenção', resumo_historico: previousRun ? `Anterior: ${previousRun.health_score} pts` : 'Iniciando histórico' }, campanhas_analise, otimizacoes_prioritarias: otimizacoes };
}
